...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        # list of dictionaries {(1, 3): 0.0053457533001673874, (1, 4): 0.0033856437567726795, (1, 5): 0.0008074314880461162}
        ...
        for ghost in livingGhostPositionDistributions:
            likelyPos = max(ghost, key=ghost.get)
            ghostCandidates[likelyPos] = ghost[likelyPos]

        mostLikely = max(ghostCandidates, key=ghostCandidates.get)

        # then choose an action that minimizes the maze distance to the closest ghost.
        actionDistance = {}
        for action in legal:
            mazeDistance = self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, action), mostLikely)
            actionDistance[action] = mazeDistance

        return min(actionDistance, key = actionDistance.get)
        # raiseNotDefined()
