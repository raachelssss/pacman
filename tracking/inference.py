...
def constructBayesNet(gameState: hunters.GameState):
    ...
    # print("mine", variableDomainsDict)
    ...
    mh = set()
    for pacPos in variableDomainsDict[PAC]:
        for ghostPos in variableDomainsDict[GHOST0]:
            mh.add(manhattanDistance(pacPos, ghostPos))

    for pacPos in variableDomainsDict[PAC]:
        for ghostPos in variableDomainsDict[GHOST1]:
            mh.add(manhattanDistance(pacPos, ghostPos))

    for i in range(max(mh), max(mh)+ MAX_NOISE + 1):
        mh.add(i)
    variableDomainsDict[OBS0] = list(mh)
    variableDomainsDict[OBS1] = list(mh)
    "*** END YOUR CODE HERE ***"
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for joinVariable in eliminationOrder:
            ...
            # if incrementallyMarginalizedJoint.unconditionedVariables() == 1:
            #     print('hello')
            #     for var in incrementallyMarginalizedJoint.variablesSet():
            #         if var in eliminationOrder:
            #             eliminationOrder.remove(var)
            #     pass
            # print('uncond', incrementallyMarginalizedJoint.unconditionedVariables())
            # for eliminationVariable in eliminationOrder: #in eliminationOrder :)
            #     if eliminationVariable in incrementallyMarginalizedJoint.unconditionedVariables():
            #
            #         incrementallyMarginalizedJoint = eliminate(incrementallyMarginalizedJoint, eliminationVariable)
            #         break
        fullJoint = joinFactors(currentFactorsList)
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        randomChoices = random.choices(list(self.keys()), weights=self.values(), k=1)
...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        tempbeliefs = self.beliefs.copy()
        alldist = {oldPos: self.getPositionDistribution(gameState, oldPos) for oldPos in self.allPositions}
        for newPos in self.allPositions:
            p = 0
            for oldPos in self.allPositions:
                p += (self.beliefs[oldPos]*alldist[oldPos][newPos])
            tempbeliefs[newPos] = p
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        # parPerPos = (numP - numP%len(self.legalPositions))/len(self.legalPositions)
        # for i in range(len(self.legalPositions)):
        #     # print(i)
        #     # print(self.getPositionDistribution(gameState, i))
        #     self.particles.append(int(parPerPos))
        self.particles = []
        "*** YOUR CODE HERE ***"

        for i in range(numP):
            evenly = self.legalPositions[i% len(self.legalPositions)]
            self.particles.append(evenly)
    ...
    def getBeliefDistribution(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        # curBelief = DiscreteDistribution()
        # for i,pos in enumerate(self.legalPositions):
        #     curBelief[pos] = self.particles[i]
        # curBelief.normalize()
        # return curBelief
        # raiseNotDefined()
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        # curBelief = self.getBeliefDistribution() #ghostPos: particleProb
        # weight = {(pos, par): 1 for pos, par in curBelief.items()}
        # for pos, par in curBelief.items():
        #     weight[(pos, par)] *= self.getObservationProb(observation, gameState.getPacmanPosition(), pos, self.getJailPosition())
        #
        # if sum(list(weight.values())) == 0 or sum(list(self.getBeliefDistribution().values())) == 0:
        #     self.initializeUniformly(gameState)
        #     return
        # for i,par in enumerate(self.particles):
        #     self.particles[i] *= list(weight.values())[i]


        weight = DiscreteDistribution()

        for par in self.particles:
            if par not in list(weight.keys()):
                weight[par] = self.getObservationProb(observation, gameState.getPacmanPosition(), par, self.getJailPosition())
            else:
                weight[par] += self.getObservationProb(observation, gameState.getPacmanPosition(), par, self.getJailPosition())

        if not weight.total() or sum(list(self.getBeliefDistribution().values())) == 0:
            self.initializeUniformly(gameState)
            return
    ...
    def elapseTime(self, gameState):
        ...
        # raiseNotDefined()
        "*** END YOUR CODE HERE ***"
        # This function should construct a new list of particles that corresponds to each existing particle in self.particles advancing a time step

        newParticles = []
        # alldist = {oldPos: self.getPositionDistribution(gameState, oldPos) for oldPos in list(self.getBeliefDistribution().keys())}
        for oldPos in self.particles:
            getDist = self.getPositionDistribution(gameState, oldPos)
            sample = getDist.sample()
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for par in self.particles:
            ...
            for i in range(self.numGhosts):
                if par not in list(weight.keys()):
                    prob *= self.getObservationProb(observation[i], gameState.getPacmanPosition(), par[i],
                                                    self.getJailPosition(i))
                    if i == self.numGhosts-1:
                        weight[par] = prob
                else:
                    prob *= self.getObservationProb(observation[i], gameState.getPacmanPosition(), par[i],
                                                    self.getJailPosition(i))
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            for i in range(len(newParticle)):
                getDist = self.getPositionDistribution(gameState, oldParticle, i, self.ghostAgents[i])
                sample = getDist.sample()
